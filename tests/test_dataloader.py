...
def test_dataloader_scaling():
    ...
    for num_agents in [2, 3, 5]:
        ...
        loader.collect_transition(obs, obs, {f"gnb_{i}": 1.0 for i in range(num_agents)}, obs, {f"gnb_{i}": False for i in range(num_agents)}, {f"gnb_{i}": False for i in range(num_agents)})
